...
if systemctl is-active --quiet hy-webhook 2>/dev/null && \
   docker ps --format '{{.Image}}' 2>/dev/null | grep -q 'remnawave-sub-hy:local'; then
    echo ""
    warn "Интеграция уже установлена."
    echo -e "  ${BOLD}1)${NC} Переустановить полностью"
    echo -e "  ${BOLD}2)${NC} Обновить только форк subscription-page"
    echo -e "  ${BOLD}3)${NC} Обновить только hy-webhook"
...
# ── Port Hopping ──────────────────────────────────────────────────
echo ""
echo -e "  ${WHITE}Port Hopping — обход блокировок по порту:${NC}"
echo "  ┌────────────────────────────────────────────────────────────────┐"

if $HAS_PORT_HOPPING; then
    printf "  │  Текущий режим: Port Hopping — порт %-28s│\n" "$MAIN_PORT + $HOP_RANGE"
    echo "  │                                                                │"
    echo "  │  1) Оставить как есть                                          │"
    echo "  │  2) Отключить Port Hopping                                     │"
else
    printf "  │  Текущий режим: один порт %-36s│\n" "$MAIN_PORT"
    echo "  │                                                                │"
    echo "  │  1) Оставить как есть                                          │"
    printf "  │  2) Включить: %-2s + 20000-29999  (рекомендуется)              │\n" "$MAIN_PORT"
    printf "  │  3) Включить: %-2s + 40000-49999                               │\n" "$MAIN_PORT"
    printf "  │  4) Включить: %-2s + 50000-59999                               │\n" "$MAIN_PORT"
    echo "  │  5) Ввести свой диапазон                                       │"
fi
echo "  └────────────────────────────────────────────────────────────────┘"

read -rp "  Выбор [1]: " hop_ch < /dev/tty
hop_ch="${hop_ch:-1}"

if $HAS_PORT_HOPPING; then
    case "$hop_ch" in
        1) info "Port Hopping оставлен без изменений" ;;
        2)
    ...
    case "$hop_ch" in
        1) info "Порт оставлен без изменений" ;;
        2) NEW_RANGE="20000-29999" ;;
        3) NEW_RANGE="40000-49999" ;;
        4) NEW_RANGE="50000-59999" ;;
        5)
...
echo -e "  Hysteria2:   ${CYAN}${HY_DOMAIN}:${HY_PORT}${NC}"
echo -e "  Sub домен:   ${CYAN}${SUB_DOMAIN}${NC}"
if $HAS_PORT_HOPPING; then
    echo -e "  Port Hopping:${CYAN} включён — диапазон ${HOP_RANGE}${NC}"
fi
echo ""
if [ -n "$WEBHOOK_SECRET_DISPLAY" ]; then
    echo -e "  ${WHITE}Webhook secret — файл /etc/hy-webhook.env:${NC}"
